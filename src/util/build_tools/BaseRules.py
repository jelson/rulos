...
class Platform:
            ...
            BUILDERS={
                "MakeLSS": Builder(
                    ...
                    action=env.SpinnerAction(
                        f"{compiler_prefix}objdump -h -S --syms $SOURCE > $TARGET", "Disassembling"
                    ),
                ),
                "ShowSizes": Builder(
                    src_suffix=".elf",
    ...
    def build(self, target):
        ...
        # Build the lib
        rocket_lib = env.StaticLibrary(
            os.path.join(build_obj_dir, "src", "lib", "rocket"), source=platform_lib_srcs
        )
        ...
        # Build the app
        target_sources = [
            os.path.join(build_obj_dir, s)
            for s in target.sources + self.platform_specific_app_sources()
        ]

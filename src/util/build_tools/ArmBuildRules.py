    ...
    def programming_cmdline(self, usbpath, elfpath):
        ...
        return (
            [
                ...
                "set confirm off",
                "-ex",
                f"target extended-remote {usbpath}",
            ]
            + power
            + [
                "-ex",
...
class ArmStmPlatform(ArmPlatform):
    def __init__(self, chip_name, extra_peripherals=[], extra_cflags=[], flash_reserve_k=0):
        ...
        if chip_name not in self.CHIPS:
            util.die(f"Unrecognized chip {chip_name}. Known chips: " + ",".join(self.CHIPS))
    ...
    class MajorFamily:
        def __init__(self, name, arch_name):
            ...
            self.sources = os.path.join(
                self.cmsis_root, "Source", "Templates", "system_" + name.lower() + "xx.c"
            )
    ...
    def platform_lib_source_files(self):
        return (
            self.arm_platform_lib_source_files()
...
class ArmNxpPlatform(ArmPlatform):
    ...
    class Chip:
        def __init__(self, name, chip_family, flashk, ramk, arch_name, driver_lib="lpc_11uxx"):
            self.name = name
    ...
    def __init__(self, chip_name, extra_peripherals=[], extra_cflags=[]):
        ...
        self.driver_lib_path = os.path.join(LPC_VENDOR_ROOT, self.chip.driver_lib)

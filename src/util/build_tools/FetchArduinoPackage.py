...
class Fetcher:
    ...
    def select_package(self):
        ...
        assert (
            len(packages) == 1
        ), f"Found {len(packages)} {self.package_name} packages when we expected 1"
        package = packages[0]
        ...
        assert (
            len(versions) == 1
        ), f"Found {len(versions)} versions of {self.package_name} v{self.version} when we expected 1"
        version = versions[0]

        return [version] + [
            self.select_tools_dependencies(tool) for tool in version["toolsDependencies"]
        ]
    ...
    def select_tools_dependencies(self, spec):
        ...
        tool_match = [
            t for t in tools if t["name"] == spec["name"] and t["version"] == spec["version"]
        ]
        assert (
            len(tool_match) == 1
        ), f"Found {len(tool_match)} matches for {spec['name']}, {spec['version']}"
        tool = tool_match[0]

        system_match = [s for s in tool["systems"] if s["host"] == self.host_platform]
        assert (
            len(system_match) == 1
        ), f"Found {len(system_match)} matches for {tool['name']}, {self.host_platform}"

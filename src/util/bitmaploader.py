...
#
# Warning: Don't try to reuse this.
#
# This encodes bitmaps into an RLE-compressed format we had to use for
# the original atmega328 where we had severe program memory constraints.
#
...
def get_bitfield(bytes, offset, len):
    ...
    print(
        "GB select value %03x from byte %d, right-shift %d"
...
class DataBlock:
    ...
    def add_field(self, sym, elevenBitField):
        ...
        gotValue = get_bitfield(
            self.data, self.index[sym].offset_bits + (self.index[sym].len - 1) * 11, 11
        )

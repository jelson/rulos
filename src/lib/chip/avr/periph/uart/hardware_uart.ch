...
void hal_uart_init_name(UartHandler *handler, uint32_t baud, r_bool stop2, uint8_t uart_id)
{
...
{
	//audioled_set(0, 1);
	if (g_uart_handler[UARTID] != nullptr && g_uart_handler[UARTID]->recv != nullptr) {
...
static inline void _handle_send_ready_name()
{
	...
	if (g_uart_handler[UARTID] != nullptr && g_uart_handler[UARTID]->send != nullptr) {
...
ISR(_USART_RXC_vect)
{
	_handle_recv_ready_name(_UDR);
}
ISR(_USART_UDRE_vect)
{
	_handle_send_ready_name();
}

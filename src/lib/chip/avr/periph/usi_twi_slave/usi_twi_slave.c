...
//  // state is set to check address mode and the counter is set to wait 8
//  clocks // (enough for the address/rw byte to be transmitted) before
//  overflowing and  // triggering the first state table interrupt.  If a stop
//  condition occurs,   // reset the start condition detector to detect the next
//  start condition.     //
/////////////////////////////////////////////////////////////////////////////////
ISR(USI_START_VECT) {
  DEBUG_STROBE(1);
  ...
  // because the Stop Condition Flag is going to be set from the last TWI
  // sequence." I'm not sure I agree with this; the USI TWI SLave applicatoin
  // note does not check USISR here.
...
//  set the counter to 1 (actually -1, or 0x0E) it will wait one clock. // This
//  is used to set up a state table of I2C transmission states that fits  // the
//  I2C protocol for proper transmission.                                  //
/////////////////////////////////////////////////////////////////////////////////
ISR(USI_OVF_VECT) {
  DEBUG_STROBE(2);
  ...
  switch (usi.state) {
    ...
    case USI_SLAVE_RECV_DATA_ACK_SEND: {
      if (usi.recv_len < 0) {
        // If we've already experienced an overflow on this packet, NACK the
        // byte.
        USIDR = 1;
      } else if (usi.recv_slot->capacity <= usi.recv_len) {
        // If the receive buffer just overflowed, record the overflow and NACK
        // the byte.
        usi.recv_len = -1;
        USIDR = 1;
      } else {
...
// Do you want to know what's stupid? The USI module in attiny hardware is

 ...
 *on the bus using the USI_TWI_Start_Transceiver_With_Data() function. If the
...
#define USI_TWI_DATA_OUT_OF_BOUND \
  0x01                             // Transmission buffer is outside SRAM space
...
#define USI_TWI_NO_ACK_ON_ADDRESS \
  0x06  // The slave did not acknowledge  the address
#define USI_TWI_MISSING_START_CON \
  0x07  // Generated Start Condition not detected on bus
#define USI_TWI_MISSING_STOP_CON \
  0x08  // Generated Stop Condition not detected on bus
...
#endif
    unsigned char
    usi_twi_master_send(const uint8_t sevbit_addr, char *, unsigned char);
unsigned char usi_twi_master_recv(const uint8_t sevbit_addr, char *,
                                  unsigned char);

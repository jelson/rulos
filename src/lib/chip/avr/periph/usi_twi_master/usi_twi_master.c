 ...
 *multi-master system. Usage             : Initialize the USI module by calling
 *the USI_TWI_Master_Initialise() function. Hence messages/data are transceived
 *on the bus using the USI_TWI_Transceive() function. The transceive function
...
union USI_TWI_state {
  unsigned char
      errorState;  // Can reuse the TWI_state for error states due to that it
                   // will not be need if there exists an error.
  struct {
...
#endif
    unsigned char
    usi_twi_master_sendorrecv(const uint8_t eightbit_addr, char *msg,
    ...
    /* Else masterRead cycle*/
      ...
      *(msg++) = USI_TWI_Master_Transfer(tempUSISR_8bit);
      ...
      if (msgSize == 1)  // If transmission of last byte was performed.
      {
        ...
        USIDR =
            0x00;  // Load ACK. Set data register bit 7 (output for SDA) low.
      }
...
unsigned char usi_twi_master_send(const uint8_t sevbit_addr, char *msg,
                                  unsigned char msgSize) {
  return usi_twi_master_sendorrecv(sevbit_addr << 1, msg, msgSize);
}

unsigned char usi_twi_master_recv(const uint8_t sevbit_addr, char *msg,
                                  unsigned char msgSize) {

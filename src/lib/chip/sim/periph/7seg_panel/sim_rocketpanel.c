...
static void sim_program_labels() {
  ...
  for (bl = g_sim_theTree; bl->label != NULL; bl += 1) {
    ...
    mvwprintw(mainwnd, bl->y, bl->x + (4 * NUM_DIGITS - strlen(bl->label)) / 2,
              bl->label);
  }
}
...
static void adc_simulator_start() {
  ...
  // create input window
  adc_input_window = newwin(4, 17, 2, NUM_DIGITS * DIGIT_WIDTH + 3);
...
static void uart_simulator_start() {
  ...
  // create input window
  uart_input_window =
      newwin(4, sizeof(recent_uart_buf) + 1, 2, NUM_DIGITS * DIGIT_WIDTH + 3);
...
void hal_init_keypad() { g_keypad_enabled = TRUE; }
...
void hal_init_joystick_button() { g_joystick_trigger_state = FALSE; }

...
// Called at task time
static void _buf_received(UartState_t *s, void *user_data, char *buf, size_t len) {
  linereader_feed((LineReader_t *)user_data, buf, len);
}

void linereader_init_unbound(LineReader_t *l, UartState_t *uart, linereader_cb cb,
                             void *user_data) {
  assert(cb != NULL);
  memset(l, 0, sizeof(*l));
  l->uart = uart;

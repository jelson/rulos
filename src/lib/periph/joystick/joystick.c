...
// R/X+R. Since the ADC is scaled from 0..1023, and R=40Kohm, then the ADC value
// will be
//      ADC = 1023 * (X/X+R).
// Solving for X gives us
//      X = (40*1023) / ADC  - 40.  (X in Kohm)
// That will give us X between 0 and 100, since the joystick's range is 0..
// 100Kohm.
...
static int8_t adc_to_100scale(uint16_t adc) {
  ...
  // and combine into a single equation (cheating using mathematica - jer
  // algebra fail):
  int32_t retval = ((int32_t)96282 / adc) - 194;

  if (retval < -99) return -99;
  if (retval > 99) return 99;
  return retval;
}
...
void joystick_poll(JoystickState_t *js) {
  ...
  if (js->x_pos < -ACTUATION_THRESHOLD) js->state |= JOYSTICK_STATE_LEFT;
  if (js->x_pos > -RELEASE_THRESHOLD) js->state &= ~(JOYSTICK_STATE_LEFT);
  if (js->x_pos > ACTUATION_THRESHOLD) js->state |= JOYSTICK_STATE_RIGHT;
  if (js->x_pos < RELEASE_THRESHOLD) js->state &= ~(JOYSTICK_STATE_RIGHT);

  if (js->y_pos > ACTUATION_THRESHOLD) js->state |= JOYSTICK_STATE_UP;
  if (js->y_pos < RELEASE_THRESHOLD) js->state &= ~(JOYSTICK_STATE_UP);
  if (js->y_pos < -ACTUATION_THRESHOLD) js->state |= JOYSTICK_STATE_DOWN;
  if (js->y_pos > -RELEASE_THRESHOLD) js->state &= ~(JOYSTICK_STATE_DOWN);

...
#define LMDELAY()	{volatile int x; for (int de=0; de<2; de++) { x+=1; }}
//#define LMDELAY()	{}

inline static void _lms_shift_byte_cols(uint8_t data)
{
	uint8_t bit;
	for (bit=0; bit<8; bit++)
	{
...
inline static void _lms_shift_byte_rows(uint8_t data)
{
	uint8_t bit;
	for (bit=0; bit<8; bit++)
	{
...
{
	_lms_shift_byte_rows(rowdata);
	STROBE(LMS_LATCH_ENABLE_ROWS);
}

void _lms_configure_col(uint16_t coldata)
{
	_lms_shift_byte_cols((coldata >> 8) & 0xff);
	_lms_shift_byte_cols((coldata >> 0) & 0xff);
...
uint8_t _lms_row_remap_table[8] = {1<<3,1<<4,1<<5,1<<6,1<<2,1<<1,1<<0,1<<7};
uint8_t _lms_bitswap[16] = {
...
void _lms_update(Activation *act)
{
	...
	if (lms->pwm_enable)
	{
		...
		gpio_set(LMS_OUTPUT_ENABLE_INV);
		uint8_t row_val = _lms_row_remap_table[actual_row];
		_lms_configure_row(row_val);
		uint16_t col_val;
		if (do_green)
		{
			uint8_t green = lms->bitmap.green[actual_row];
			col_val = _lms_bitswap[green&0xf] | ((green&0xf0)<<8);
		}
		else
		{
			uint8_t red = lms->bitmap.red[actual_row];
			col_val = (_lms_bitswap[red&0xf]<<4) | ((red&0xf0)<<4);
		}
		_lms_configure_col(col_val);
		gpio_clr(LMS_OUTPUT_ENABLE_INV);

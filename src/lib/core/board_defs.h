...
// x, y tell the simulator where to place the board display on the ncurses canvas.
// remote_addr is the address of the dongle that owns this board, specified as an offset from DONGLE_BASE_ADDR
// remote_idx is the index (board select configuration) of the board on that remote dongle
#define B_MISSION_CLOCK         DBOARD("Mission Clock",         PG PG PG PG PG PG PY PY,  3,  0,  0, 0)
#define B_LUNAR_DISTANCE        DBOARD("Distance to Moon",      PR PR PR PR PR PR PR PR, 33,  4,  1, 1)
...
#define B_THRUSTER_ACT          DBOARD("Thruster Actuation",    PB PR PR PR PB PY PY PY,  3, 25,  7, 7)
#define B_AZE                   DBOARD("Azimuth, Elevation",    PG PG PG PR PR PY PY PY, 33, 0 ,  8, 0)
#define B_LIQ_HYD_PRES          DBOARD("Liquid Hydrogen Prs",   PG PG PG PG PG PG PG PG,  3, 4 ,  9, 1)
#define B_FLIGHTCOMPUTER0       DBOARD("Flight Computer",       PR PR PR PR PY PY PY PY, 17, 9 , 10, 2)

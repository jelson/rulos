...
static gint drawLed(int x, int y, int rColor, int gColor) {
  ...
  gdk_draw_arc(canvas, canvas_gc, TRUE, LED_SPACING / 2 + x * (LED_SPACING + LED_RADIUS),
               LED_SPACING / 2 + y * (LED_SPACING + LED_RADIUS), LED_RADIUS, LED_RADIUS, 0,
               360 * 64);

  gdk_gc_set_rgb_fg_color(canvas_gc, &ledOutline);

  gdk_draw_arc(canvas, canvas_gc, FALSE, LED_SPACING / 2 + x * (LED_SPACING + LED_RADIUS),
               LED_SPACING / 2 + y * (LED_SPACING + LED_RADIUS), LED_RADIUS, LED_RADIUS, 0,
               360 * 64);
}

static gint initCanvas() {
  canvas = gdk_pixmap_new(topWindow->window, topWindow->allocation.width,
                          topWindow->allocation.height, -1);

  canvas_gc = gdk_gc_new(canvas);
  GdkColor black = {0, 0, 0, 0};
  gdk_gc_set_rgb_fg_color(canvas_gc, &black);
  gdk_draw_rectangle(canvas, canvas_gc, TRUE, 0, 0, topWindow->allocation.width,
...
int main(int argc, char *argv[]) {
  ...
  gtk_window_set_default_size(GTK_WINDOW(topWindow), X_LEDS * (LED_RADIUS + LED_SPACING),

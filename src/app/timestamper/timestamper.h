...
// Cross-file interface between timestamper.c (which owns the channel state and the TIM2 hardware)
// and scpi.c (which translates SCPI commands into calls on this API). Mirrors the pulsegen.h
// pattern.
...
// traffic, no drain cost). Every emitted record carries an exact, hardware-latched polarity bit
// regardless of slope.
//
// Emission order: each (channel, polarity) sub-stream is emitted in time order, but sub-streams are
// not interleaved into one globally ordered sequence -- in BOTH mode a channel's rising and falling
// records arrive in independent service-sized batches, just as records from different channels
// always have. Hosts that want total order sort by timestamp.
typedef enum {
...
// Per-channel divider: report only every Nth pulse. N=1 reports every pulse. N=0 is rejected. The
// divider count is reset to 0 on every set so the next pulse starts a fresh group.
void timestamper_set_divider(int ch, uint32_t n);
uint32_t timestamper_get_divider(int ch);

// Continuous timestamp output enable. When disabled, hardware capture keeps running and the
// internal buffer keeps filling, but nothing streams to USB and missed-pulse reports are suppressed
// -- so a host can issue SCPI queries and receive clean, uncluttered responses. Default true at
// boot.
void timestamper_set_stream_enabled(bool enabled);
bool timestamper_get_stream_enabled(void);

// Output stream format. TEXT is the documented default (ASCII "<chan> <sec>.<ns> <+|->\n" per
// timestamp, where the final column is the edge polarity: '+' rising, '-' falling); BINARY emits a
// fixed TIMESTAMPER_BINARY_RECORD_LEN-byte record per timestamp -- the same 8-byte representation
// used in the device's internal ring buffer:
...
// Every record is exactly 8 bytes, timestamp or not. Counter bit [28] marks a "special message"
// instead of a timestamp: device metadata that would be a "#" comment line in text mode, carried
// in-band so binary readers see it too. When set, counter [7:0] is the message type, bit [29] is
// zero, and the seconds word is its payload:
...
// next captured pulse will be the first one the host sees. Channel configuration (slope, divider,
// format, stream enable) is preserved.
void timestamper_discard_pending(void);

#define TIMESTAMPER_FW_VERSION "0.22.0"

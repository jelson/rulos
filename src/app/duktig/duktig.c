...
static void shift_in_config(DuktigState_t *duktig) {
  ...
  // tidy up (not actually necessary, but makes logic analyzer output easier to
  // read)
  gpio_clr(LED_DRIVER_SDI);
}
...
static void duktig_update(DuktigState_t *duktig) {
  ...
  if (but1_pushed || but2_pushed) {
    ...
    // if we just turned the lights on, set a long timeout; if we just turned
    // them off, set a short one

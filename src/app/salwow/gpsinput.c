...
#define INVALID(m)	{ \
	...
	uint8_t num_fields =
		split(gpsi->sentence, field_index, sizeof(field_index)/sizeof(field_index[0]));
	// $GPGGA,235453,4736.760,N,12219.740,W,1,05,2.2,104.1,M,-18.4,M,,*7A
	if (num_fields!=14) { INVALID("num_fields"); }
#define FI_TIME			0
#define FI_LAT			1
...
#define FI_QUALITY		5
	...
	if (memcmp(&gpsi->sentence[field_index[FI_LAT_SIGN]], "N,", 2)!=0) { INVALID("N"); }
	if (memcmp(&gpsi->sentence[field_index[FI_LON_SIGN]], "W,", 2)!=0) { INVALID("W"); }
	if (memcmp(&gpsi->sentence[field_index[FI_QUALITY]],  "1,", 2)!=0) { INVALID("Quality"); }
	bool rc;
	rc = _parse_ddm(&gpsi->sentence[field_index[FI_LAT]], &gpsi->lat);
	if (!rc) { INVALID("lat ddm"); }
	rc = _parse_ddm(&gpsi->sentence[field_index[FI_LON]], &gpsi->lon);
	if (!rc) { INVALID("lon ddm"); }
